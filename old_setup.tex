...
\noindent The calibration procedure relies on the assumption that all sensors in the capsule are at the same temperature. This limits the number of RTDs in a single run to four, since i) they should be as close as possible to each other and ii) as far as possible from the capsule walls (if sensors were placed too close to the wall their measurements could be biased). Two different methods, described schematically in Fig.~\ref{fi:CAL_sequence}, were used to cross-calibrate all 48 sensors in runs of 4 sensors:
...
\centering
\begin{tabular}{ c }
\includegraphics[height=0.34\textheight]{./figure_8_a.jpg}
\end{tabular}%
\begin{tabular}{ c }
...
\noindent The reported uncertainties presented are, on average, below 1~mK. They correspond to the standard deviation of the four independent measurements used to determine each calibration constant, combined with the error propagation from the necessary corrections: time-walk correction in the reference method, and addition of calibration constants for the tree method. These corrections may include unknown systematic effects that impact the final calibration constants. Such effects must be considered together with the systematic uncertainty arising from the assumption of homogeneous temperature inside the capsule and the potential uncorrected electronic residual offset. 

To estimate the overall precision of the calibration, a cross-check was performed. The calibration strategy provides two independent estimates of each sensor’s calibration constant: one from the reference method and one from the tree method. For the sixteen sensors included in the second round of the calibration tree, these two values are linearly independent because they rely on different datasets. Specifically, the reference method uses only first-round runs, while the tree method is based on second and third-round runs. This separation makes these sixteen sensors suitable for estimating the precision of the entire calibration procedure by comparing both methods.

By design, the difference between the calibration constants obtained from the two methods should match the offset between sensors 39645 and 39606, defined as

\begin{equation}
\Delta T_{45,06} = \Delta T_{s,06}-\Delta T_{s,45} = T_{45}-T_{06}
\end{equation}

This offset, measured directly, is $-25.1 \pm 0.7$ mK (see 10th column in Fig.~\ref{fi:CAL_sequence} and \ref{fig:offsets_tree_1}). Fig.~\ref{fig:crosscheck} shows this benchmark for the 16 sensors without (left) and with (right) time walk corrections for the 16 sensors aforementioned, probing the self-consistency of this correction. The standard deviation of this distribution, 2.4 mK, represents the quadratic sum of the total uncertainty of from both methods. Assuming that the reference sensor method carries the larger uncertainty (due to the time walk correction and the cross-reference of the two reference sensors, 39606 and 39656), we infer an upper limit of 1.7 mK for the total error of the tree method. In the same way, this value represents a lower limit for the total error of the reference method. In both cases, these uncertainties are less that half of the requirement for the temperature monitoring system in DUNE FD-HD.

It can also be seen in Fig.~\ref{fig:crosscheck} that sensors directly calibrated with respect to reference 39656 tend to lie systematically above the benchmark value. This behaviour suggests a potential bias in the reference method during the computation of $\Delta T_{56,06}(N=0)$, presented in the previous section. This observation further reinforces the conclusion that the tree method is more robust and was therefore adopted for ProtoDUNE-SP operations.

\begin{figure}[htbp]
\centering
{\includegraphics[width=0.49\textwidth]{./figure_15_b.pdf}}
{\includegraphics[width=0.49\textwidth]{./figure_15_a.pdf}}
